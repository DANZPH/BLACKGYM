...
$sql = "SELECT EndDate, Status, Balance FROM Membership
        JOIN Members ON Members.MemberID = Membership.MemberID
        WHERE Membership.MemberID = ?";
$stmt = $conn1->prepare($sql);
$stmt->bind_param("d", $memberID); // Binding MemberID parameter
$stmt->execute();
$stmt->bind_result($endDate, $membershipStatus, $balance);
$stmt->fetch();
$stmt->close();
                ...
                <li>
                    ...
                    <span class="text">
                        ...
                        <?php if ($membershipStatus === 'Active'): ?>
                            <div>
                                <p><strong>Until:‎  </strong><small><?php echo date('d M Y', strtotime($endDate)); ?></small></p>
                                <p><strong>Ends:‎ </strong><small><?php echo $remainingTime; ?></small> <span></span></p>
                            </div>
