...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($amountPaid < $amount) {
        echo json_encode(['status' => 'error', 'message' => 'Amount paid cannot be less than the amount.']);
        exit();
    }
    ...
    if ($stmt->execute()) {
        ...
        if ($updateMembershipStmt->execute()) {
            ...
            if ($updateMemberStmt->execute()) {
                ...
                echo json_encode(['status' => 'success', 'message' => 'Payment processed and membership status updated to Active!']);
            } else {
